...
class Trainer:
    ...
    def run_all_models_single_data(self, models: dict, dataset: FairDataset, **kwargs):
        ...
        for model_name, model in models.items():
            lst_res.append(self.run_single_model(model, model_name, dataset, **kwargs))
        df_res = pd.concat(lst_res).reset_index(drop=True)
        return df_res
    ...
    def run(self, models: dict, datasets: dict, verbose_header: bool = True, **kwargs):
        ...
        for dataset_name, dataset in datasets.items():
            lst_res.append(self.run_all_models_single_data(models, dataset, **kwargs))
        self.df_res = pd.concat(lst_res).reset_index(drop=True)
        return self.df_res
